...
第一步，与所有的 IOC 容器类似，定义接口和实现类，如下所示:
...
第二步，类似于 spring 的 xml 时代，定义组件的依赖关系。在项目源码的 resources 目录下，建立 META-INF/plexus/components.xml 文件，按如下内容配置该文件：
...
其中，每个组件配置一个 `<component>` 元素，在 `<component>` 元素内，可以配置三个子元素，如下所示：
